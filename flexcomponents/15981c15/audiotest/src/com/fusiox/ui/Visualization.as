	...
	[Style(name="audioFillColor",type="Number",format="Color",inherit="no")]
 	public class Visualization extends UIComponent {
        ...
        public function set type( value:String ):void {
  		...
  		public function set bars( value:uint ) {
  			_bars = value;
  		}
		...
		private function drawWave(modifier:Number=1):void {
			var v:Number;
   			...
   			for (var i:uint = 0; i < 256; i++) {
		    	...
		    	if(type=="wave"){bitmapData.fillRect( new Rectangle(i*_rp, Math.min(v,_rc), _rp, Math.abs(_rc-v)), 0xFF000000 | _audioFillColor );}
   				bitmapData.fillRect( new Rectangle(i*_rp, v, _rp, 1+Math.abs(lastv-v)), 0xFF000000 | _audioLineColor );
		    	lastv=v;
		    }
		}
		...
		private function drawEQ():void {
			var v:Number;
   			...
   			for (var i:uint = 0; i < _bars; i++) {
		    	...
		    	bitmapData.fillRect( new Rectangle(nrp/8+i*nrp, v, nrp/2+nrp/4, _rc*2-v), 0xFF000000 | _audioFillColor);
